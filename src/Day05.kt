...
private fun part1(input: List<String>): Int {
    val lines = input.map(Line::fromString).filter { it.isHorizontal || it.isVertical }
    return getMostDangerousPoints(lines, 2).size
}

private fun part2(input: List<String>): Int {
    val lines = input.map(Line::fromString).filter { it.isHorizontal || it.isVertical || it.isDiagonal }
    return getMostDangerousPoints(lines, 2).size
}
